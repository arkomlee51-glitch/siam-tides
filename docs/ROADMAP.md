...
- [x] เชื่อมต่อใหม่กับเกมหลายคน (in-process, ไม่ต้องมี Redis จริง) — ทดสอบ 4 คนต่อ WS พร้อมกันจริง ๆ,
      ...
      state ล่าสุดถูกต้องครบทุกคำสั่งที่พลาดไประหว่างหลุดสาย ไม่ตกหล่น รวมถึง `resync` ระหว่างสายไม่หลุดเต็ม ๆ
      ด้วย (`apps/server/test/multiplayer-reconnect-ws.test.ts`) **ยังไม่ทำ**: ยังไม่ได้ทดสอบข้าม server
...
การทูตพื้นฐาน, ending, cold-start replay, WebSocket ต่อ/หลุด/ต่อใหม่กับ 4 คนพร้อมกัน) ทดสอบผ่านหมดแล้ว

## เฟส 6 — เนื้อหาเต็ม (เริ่มแล้ว)

- [x] **schema เนื้อหาต่อบท** (`packages/engine/src/content/schema.ts`) — `ChapterDefinition` ครอบคลุม
      แผนที่/ภูมิประเทศ/ฤดูกาล/อาคาร/ค่าใช้จ่าย/กฎบาลานซ์/perk/มหาอำนาจต่างชาติ/ข้อเสนอ/ตอนจบ/ที่นั่งเริ่มต้น
      พร้อม `validateChapterDefinition()` ตรวจโครงสร้าง (ไม่ตรวจบาลานซ์) คืนปัญหาทั้งหมดที่เจอในครั้งเดียว —
      resource id (`rice`/`man`/`wealth`/`faith`/`know`) ตรึงคงที่ข้ามบททุกบท ดู [ADR-0007](adr/0007-chapter-content-schema-and-legacy.md)
      ข้อ 2 ว่าทำไม
- [x] **ระบบ Legacy** (`packages/engine/src/content/legacy.ts`) — 6 หมวด (infrastructure/prosperity/culture/
      knowledge/military/diplomacy) แต่ละหมวดมีเพดาน 10–15% กันไม่ให้ผู้เล่นที่เก่งบทแรกครองบทหลัง ๆ แบบ
      ไม่มีทางแพ้ (ดู ADR-0007 ข้อ 4–5) คำนวณจากสถิติทั่วไปของ faction ตอนจบบท ไม่อ้างอิง building/perk id
      เฉพาะบท จึงใช้ข้ามบทได้โดยไม่ต้องมี mapping table — ฟังก์ชัน pure ทั้งหมด (`computeLegacyBonuses`/
      `mergeLegacyBonuses`/`applyLegacyBonuses`) ทดสอบครบว่าไม่มีทางเกินเพดานแม้ input สุดขั้ว และคำนวณแบบ
      deterministic (`packages/engine/test/content-schema.test.ts`, 10 เคส)
- [x] **พอร์ตเนื้อหาที่มีอยู่จริงเป็นบทตัวอย่าง** — `chapters/early-rattanakosin.ts` derive จาก `data.ts`
      ...
      ประวัติศาสตร์**
- [~] **rewire เอนจิน — gameplay logic อ่านจาก chapter เกือบทั้งหมดแล้ว, เหลือเรขาคณิตแผนที่กับโครงเหตุการณ์
  ประจำฤดู (ตั้งใจเลื่อน)**: `createGame` (`state.ts`) รับ `chapter?: ChapterDefinition` อ่าน seats/
  ...
  เปลี่ยนหน้าตา** → ระบบ effect ทั่วไป: `PerkDefData.effects` (Addendum 4), `BuildingDefData.effects`,
  `TerrainDefData.disasterExposure`, กลไกไผ่ลู่ลมที่วนตามมหาอำนาจที่ chapter ลงทะเบียนจริงไม่ผูก `lion`/
  `eagle` (Addendum 5), แล้ว `endings.ts`/`actions.ts`/`views.ts`/balance rules ของ `ai.ts` + ฟิลด์ใหม่
  `newCityNames` (Addendum 6) — `GameState.chapterId` + registry (`content/chapters/index.ts`) ให้ทุกจุด
  lookup เนื้อหาบทได้จริง (engine 63 เทสต์ผ่านหมด รวม 20 เคสที่พิสูจน์ด้วยชื่อ/ค่าที่เอนจินไม่เคยรู้จักว่าไม่ได้
  ผูกกับ `data.ts` อีกแล้ว) — **ตั้งใจเลื่อน**: (1) `hex.ts`/`movement.ts` ยังอ่านแผนที่จาก `data.ts` เป็น
  singleton — กระทบ renderer เว็บ, เสี่ยงชนกันระหว่างหลายเกมบนเซิร์ฟเวอร์ และยังไม่รู้ว่าบทอื่นจะมีแผนที่ของ
  ตัวเองจริงไหม (Addendum 6); (2) โครงเหตุการณ์ประจำฤดูใน `turn.ts`'s `seasonalEvents` (Addendum 5) — ทั้งสอง
  ข้อรอเนื้อหาบทจริงอีกอย่างน้อยหนึ่งบทก่อนออกแบบ
...
- ระวังการนำเสนอรัฐเพื่อนบ้านอย่างเป็นกลาง ต้องคำนึงถึงตลอดตอนเขียนเนื้อหาจริง (ยังไม่เริ่ม)
